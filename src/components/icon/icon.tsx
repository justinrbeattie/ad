...
export default component$((props:{ unicode: string, color: 'current'|'brand'| 'success'|'info'|'warning'|'danger', size: '16px'| '24px'| '32px'| '48px'|'64px', role: string, ariaLabel: string }) => {
  ...
  return (
    <i style={{ 'font-size': props.size, 'width': 'calc(' + props.size + ' + 8px )', 'height': 'calc(' + props.size + ' + 8px )', 'color': ' var(--' + props.color + ') ' }}  role={props.role} arial-label={props.ariaLabel}  ref={iconRef}>
